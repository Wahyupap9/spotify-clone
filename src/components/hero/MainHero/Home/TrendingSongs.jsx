    ...
    <>
      ...
      <div className="flex gap-2 overflow-x-scroll overflow-y-hidden h-fit ">
        {songsData.map((songItem) => (
          <SongItem key={songItem.id} songItem={songItem} />
        ))}
      </div>
    </>
  );
...
const SongItem = ({ songItem }) => {
  ...
  return (
    <div className="relative flex flex-col flex-none rounded p-4 w-44 md:w-56 h-fit hover:bg-grayBg transition-all group">
      <img src={songItem.image} className="w-56" alt="" />
      <h1 className="text-grayText font-semibold mt-3">{songItem.name}</h1>
      <button
        onClick={() =>
          isPlaying && song.id === songItem.id ? pause() : playByID(songItem.id)
        }
